...
void logData() {
  ...
  while (1) {
    ...
    if (Serial.available()) {
#else
    if ((millis() - starttime) > AUDIO_DURATION) {
#endif
      // Stop ISR calls.
      adcStop();
...
#else
  if ((millis() - logStart) > LOG_INTERVAL) {
    digitalWrite(greenLEDpin, HIGH);
    Serial.print("Starting new log at ");
    Serial.println(millis());
    logMeta();
    logData();
    delay(100);

    // wait for logData to finish
    while (log_running) {};

    digitalRead(greenLEDpin, LOW);
    logStart = millis();
    Serial.println("New datalog will begin in ");
